...
- RecordRequest payment escrow, refund processing, and fulfillment are implemented: payable requests can move from `PENDING` to `PAID/HELD`, held payments can be marked `REFUND_PENDING/REFUNDED` before fulfillment, fulfillment creates a signed `Credential` linked to the request, publishes it into the learner passport, releases held payment, and records revenue/refund ledger entries.
...
- Production database scaling is documented in `docs/runbooks/production-database-scaling.md`, with Supabase/PostgreSQL retention, partitioning, read-replica, index, and query rules for national-scale operation.
...
- RecordRequest fulfillment and refund processing validate with `npm run db:generate`, `npm run db:deploy`, `npm run typecheck`, `npm test`, and `npm run smoke:api`; contract is documented in `docs/api/record-request-fulfillment-contract.md`.
