...
- Sealed-session reopen escalation now exists in `GovernanceService`, with institution request, durable `SealedSessionReopenRequest` queue state, one-open-request-per-session protection, 72-hour due dates, Founder-only approve/reject, session status update on approval, and audit events.
...
- Founder Console v5 setup-health now includes missing grading rules, missing subjects/courses, staff assignment gaps, slow/failed validation or upload jobs, storage object counts, transfer alerts, and disputed rollovers.
...
- RecordRequest escrow and fulfillment now exist: payment confirmation holds funds, fulfillment publishes a signed historical record credential into the learner passport, and payment release creates a credential export revenue ledger entry.
